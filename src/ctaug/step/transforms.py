...
class StepTransform(DictTransform):
    ...
    def __init__(
        ...
        cutoff_index: int = -1,
        cut_off_pixel_value_weight: Union[Tuple[float, float], List[float]] = (
            0.2,
            0.6,
        ),
        ...
        self.augmentor = StepAugmentation(
    ...
    def __call__(self, **data_dict: Any) -> Dict[str, Any]:
        ...
        if data.shape != initial_size:
            raise ValueError(
                f"Error in StepTransform for size, {data.shape} and {initial_size}"
            )
        if to_torch:
...
class MotionTransform(DictTransform):
    ...
    def __init__(
        ...
        self.key_target = key_target
        self.key_origin = key_origin
        self.augmentor = MotionAugmentation(
            motion_move_position=motion_move_position,
    ...
    def __call__(self, **data_dict: Any) -> Dict[str, Any]:
        ...
        initial_size = data.shape
        data, seg, info = self.augmentor(
            data, seg, random.choice(["zero", "mean", "cut_resize"])
        )
        if data.shape != initial_size:
            raise ValueError(
                f"Error in MotionTransform for size, {data.shape} and {initial_size}"
            )
        if to_torch:
            data = torch.from_numpy(data)
        if seg_to_torch:
...
class StepMotionTransform(DictTransform):
    ...
    def __call__(self, **data_dict: Any) -> Dict[str, Any]:
        ...
        if data.shape != initial_size:
            raise ValueError(
                f"Error in StepMotionTransform for size, {data.shape} and {initial_size}"
            )
